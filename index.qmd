...
While working on the {[kitems](https://github.com/thekangaroofactory/kitems)} [@kitems] package which aims at providing a framework to manage tabular data inside R Shiny applications and has its main component delivered as a Shiny module server, I had to think about several implementations for this module to be used in complex apps.
...
But sometimes (actually quite often), you need more flexibility over the items or maybe you want to implement custom UI components and this will be based on a mixed implementation where both the {[kitems](https://github.com/thekangaroofactory/kitems)} module server and the parent app or module server(s) will interact with the data and with each other.
...
This guide is meant to not only be the foundation of {[kitems](https://github.com/thekangaroofactory/kitems)} module server communication, but also to extend beyond this framework and be used in any complex Shiny application.
