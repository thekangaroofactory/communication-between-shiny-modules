...
Although this is the easy part of the topic as the communication inside a module is not different from the communication inside a standard server / UI Shiny app, it is important to understand how it actually works to extend it to more complex cases in a second step.

Not different? Well, actually it is not different in the code that you will write to manage inputs / outputs but there is a major difference with the namespace concept and it is important to understand the details of it to be able to handle more complex communications and architectures.

## Calling the module server function

Whenever a module server is launched using the `moduleServer` function, `input` and `output` arguments are automatically handled and somehow *reduced* (in the sense of limited) to the scope of the module that is represented by its namespace itself designated by its `id`.
...
The `childScope` object is build based on the `id` that is passed to the `moduleServer`, so the function defining the module server logic is basically called with its `input` / `output` arguments limited to the scope defined by the `id`.
...
This is the reason why a module server can't see outside of it is own objects.
...
When the numeric input object is created, its `id` is valuated using a `ns` function defined as `NS(id)` that you will find in many documentations & articles. This `ns` is just a wrapper to avoid copying the `id` parameter of the module server function to each and every input defined in it.\
...
You can have a look at the function reference for more details [@namespacedInputsOutputs] about it.
...
I believe this is one of the main reasons why Shiny module communication feels super complicated while it is actually quite simple.
...
Again, it is a good exercise to inspect the HTML output from the browser:
...
The namespace process is either implicit or explicit through the use of the `NS` function – *which may create a bit of confusion* – but you have to keep in mind that it applies in any case so that thinking module means thinking namespace & scope.
