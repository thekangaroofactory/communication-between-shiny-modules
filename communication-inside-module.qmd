...
If you have a look at the `moduleServer` function[^summary-1] you will see that the second argument called `module` is later passed to the `callModule` function that itself defines a `childScope` object used to call the module function.[^summary-2]\
...
[^summary-1]: You can do this with a CTRL + left click on the function's name in your code.

[^summary-2]: Indeed as module = function(), passing module just pass the reference to the function while module() actually calls the function.
...
It is literaly same as using `NS(id, "numeric")` in our case.
...
You can see the function reference for more details [NS](https://shiny.posit.co/r/reference/shiny/latest/ns.html) about it.
...
The key here is to keep in mind that this wrapping / unwrapping process does take place in both ways but in different places and through different means. Wrapping is explicitely done at each input's level using the `NS` function while unwrapping is implicitly performed at the `moduleServer` function level.
...
But it is seen by **both** the module and the main servers with its encapsulated (wrapped into its namespace) name[^summary-3]:

[^summary-3]: Displaying the outputs to the console has been done thanks to this answer [@getShinyOutputs]
